...
회원 연구 글은 `Post.authorId`로 작성자와 연결한다. 탈퇴하면 Supabase Auth 사용자와 애플리케이션 회원정보 및 임시저장 글을 삭제하고 공개 글은 작성자 연결을 제거한 채 보존한다. 소셜 로그인용 계정 연결 모델과 편집자 역할은 아직 없다. `20260831000000_add_supabase_auth` migration의 Production 적용과 기존 연구자 전환은 별도 승인이 필요하다.
